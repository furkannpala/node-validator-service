...
class AfcTdTestDaoImpl extends BaseDao {

    sqlFor(options) {
        const key = JSON.stringify(options || {});
        this.cache = this.cache || {};
        if (!this.cache[key]) this.cache[key] = buildInsert("afc_td_test", options);
    ...
    insert(conn, trx, options, sessionId) {
        return this.execIgnoreDuplicate(
            conn, this.sqlFor(options), bindFor("afc_td_test", trx, options), sessionId);
    }
}

module.exports = new AfcTdTestDaoImpl();
