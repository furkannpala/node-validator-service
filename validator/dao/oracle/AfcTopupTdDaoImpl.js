...
class AfcTopupTdDaoImpl extends BaseDao {

    sqlFor(options) {
        const key = JSON.stringify(options || {});
        this.cache = this.cache || {};
        if (!this.cache[key]) this.cache[key] = buildInsert("afc_topup_td", options);
    ...
    insert(conn, trx, options, sessionId) {
        return this.execIgnoreDuplicate(
            conn, this.sqlFor(options), bindFor("afc_topup_td", trx, options), sessionId);
    }
}

module.exports = new AfcTopupTdDaoImpl();
