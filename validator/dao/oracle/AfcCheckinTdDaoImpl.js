...
class AfcCheckinTdDaoImpl extends BaseDao {

    sqlFor(options) {
        const key = JSON.stringify(options || {});
        this.cache = this.cache || {};
        if (!this.cache[key]) this.cache[key] = buildInsert("afc_checkin_td", options);
    ...
    insert(conn, trx, options, sessionId) {
        return this.execIgnoreDuplicate(
            conn, this.sqlFor(options), bindFor("afc_checkin_td", trx, options), sessionId);
    }
}

module.exports = new AfcCheckinTdDaoImpl();
